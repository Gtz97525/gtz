...
import javax.swing.*;
/**
 *	Title: makeSureNoodels.java
 *	Description: This class is make users make sure whether they want to order that noodle.
 *	@author  Qiaokun Wen
 *	@version 1.0
 */
public class makeSureNoodels extends JFrame implements ActionListener {
	...
	public String[] s;
	public int num;
	public String name,noodelsPrice;
	static int allNoodelsprice;
	
	/**
	 * use name and add two button ,use the method to write.
	 */
	public makeSureNoodels(String name){
		this.name=name;
		frame8=new JFrame();
		panel1=new JPanel();
		panel2=new JPanel();
	...
	public void actionPerformed(ActionEvent event){
		...
		if (event.getSource().equals(button1)){
			...
			try {
				...
				fileReader = new FileReader("..\\..\\Files\\NoodelsDishes.txt");
			...
			for (j = 0; j <= i; j++) {
				if ( s[j].equals(name) ) {
					...
					if (num < 1) {
						...
						new Ajoke(0);
						//System.out.println(noodelsPrice);
						set(name);
						settwo(noodelsPrice);
						break;
					}
			...
			try {
				writer = new FileWriter("..\\..\\Files\\NoodelsDishes.txt");
	 ...
	 */
	public String settwo(String noodelsPrice){
	     try{
	      String data = noodelsPrice;
	      File file =new File("ordered.txt");
	      file.createNewFile();
	      FileWriter fileWritter = new FileWriter(file.getName(),true);
	             BufferedWriter bufferWritter = new BufferedWriter(fileWritter);
